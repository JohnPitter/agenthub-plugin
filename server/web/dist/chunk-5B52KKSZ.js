...
var DATA_DIR = join(homedir(), ".agenthub-local");
if (!existsSync(DATA_DIR)) mkdirSync(DATA_DIR, { recursive: true });
var DB_PATH = join(DATA_DIR, "db.sqlite");
...
var taskLogs = sqliteTable("task_logs", {
  id: text("id").primaryKey(),
...
var agentMemories = sqliteTable("agent_memories", {
  id: text("id").primaryKey(),
  agentId: text("agent_id").notNull(),
  taskId: text("task_id"),
  type: text("type").notNull().default("learning"),
  // learning | correction | pattern | context
  content: text("content").notNull(),
  source: text("source"),
  // task execution, user feedback, self-reflection
  createdAt: integer("created_at").notNull()
});
...
var schema = { projects, agents, tasks, taskLogs, messages, integrations, agentMemories, docs };
